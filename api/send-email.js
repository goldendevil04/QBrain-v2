...
export default async function handler(req, res) {
  ...
  // Check env vars early for debugging
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.error('Missing SMTP env vars: SMTP_USER or SMTP_PASS not set');
    return res.status(500).json({ success: false, message: 'Server configuration error' });
  }
  console.log('SMTP env vars present, proceeding...');

  // Create transporter with Hostinger SMTP (fixed: removed requireTLS, as it's for STARTTLS only)
  const transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST || 'smtp.hostinger.com',
    port: parseInt(process.env.SMTP_PORT || '465'),
    secure: true, // Use TLS for port 465
    auth: {
      user: process.env.SMTP_USER, // no-reply@qbrain.in
      pass: process.env.SMTP_PASS  // Nur4Hax@786
    },
    tls: {
      ciphers: 'SSLv3',
  ...
  try {
    ...
    // Helper to send mail with Promise wrapping (ensures resolution in serverless)
    ...
    switch (type) {
      case 'contact':
        ...
        // Send to admin
        await sendEmail({
          from: `"Qbrain Website" <${process.env.SMTP_USER}>`,
    ...
    console.error('Error stack:', error.stack); // For deeper Vercel logs
    const debugInfo = process.env.NODE_ENV === 'development' ? error.message : undefined;
